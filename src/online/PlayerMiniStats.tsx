...
export function PlayerMiniStatsRow({ stats, className }: { stats: Stats | null | undefined; className?: string }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 text-[10px] leading-none whitespace-nowrap ${className ?? ""}`}>
      <span className="inline-flex items-center gap-0.5 text-orange-500" title="Nivell">
        <Star className="w-2.5 h-2.5" /> {stats.level}
      </span>
      <span className="inline-flex items-center gap-0.5 text-primary" title="% Victòries">
        <Trophy className="w-2.5 h-2.5" /> {winRate}%
      </span>
      <span className="inline-flex items-center gap-0.5 text-background/50" title="Abandonades">
